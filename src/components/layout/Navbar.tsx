  ...
  return (
    <>
      ...
      <div className="bg-[#f4f4f2] text-neutral-600 text-xs px-4 py-1.5 border-b border-neutral-200/80 flex flex-wrap items-center justify-between gap-2">
        ...
        {/* Top Bar Quick Helper */}
        <div className="flex items-center gap-3 text-[11px] font-mono">
          {!user ? (
            <button
              onClick={() => handleQuickDemo('usr_student_1')}
              disabled={loadingDemo}
              className="text-neutral-700 hover:text-neutral-950 font-semibold flex items-center gap-1 transition-colors"
              title="Instantly explore the student portal with pre-loaded milestones"
            >
              <span>⚡ Try 1-Click Student Demo</span>
            </button>
          ) : (
            <div className="flex items-center gap-2 text-neutral-600">
              <span className="text-neutral-400">Viewing as:</span>
              <span className="font-semibold text-neutral-900">{user.name}</span>
              <span className="text-neutral-300">·</span>
              <button
                onClick={() => handleQuickDemo(user.id === 'usr_student_1' ? 'usr_student_2' : 'usr_student_1')}
                disabled={loadingDemo}
                className="text-neutral-800 hover:underline font-semibold"
                title="Switch between in-progress intern and certified graduate"
              >
                Switch to {user.id === 'usr_student_1' ? 'Maya (Graduated)' : 'Alex (In-Progress)'}
              </button>
            </div>
          )}
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-14 items-center">
            ...
            <Link href="/" className="flex items-center gap-2.5 group">
              ...
              <div>
                <div className="font-semibold text-neutral-900 text-sm tracking-tight leading-none group-hover:text-neutral-700 transition-colors font-mono">
                  STANFORD<span className="text-neutral-500 font-normal">CS LAB</span>
                </div>
              ...
              {user ? (
                <div className="flex items-center gap-3 pl-3 border-l border-neutral-200">
                  {pathname !== '/student' && (
                    <Link
                      href="/student"
                      className="px-3 py-1.5 rounded-md bg-neutral-900 hover:bg-neutral-800 text-white text-xs font-medium transition-colors flex items-center gap-1.5 shadow-2xs"
                    >
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="text-left">
                      ...
                      <div className="text-[10px] text-neutral-500 font-mono capitalize flex items-center gap-1">
                        <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                        {user.role === 'student' ? 'Enrolled Intern' : 'Researcher'}
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => handleQuickDemo('usr_student_1')}
                    disabled={loadingDemo}
                    className="px-3 py-1.5 rounded-md text-xs font-semibold border border-neutral-300 bg-white hover:bg-neutral-50 text-neutral-800 transition-colors flex items-center gap-1.5 shadow-2xs"
                  >
                    <Sparkles className="w-3.5 h-3.5 text-amber-500" />
                    <span>{loadingDemo ? 'Loading...' : '1-Click Demo'}</span>
                  </button>
                  <Link
                    href="/login"
                  ...
                  <Link
                    href="/signup"
                    className="px-3.5 py-1.5 rounded-md text-xs font-medium bg-neutral-900 text-white hover:bg-neutral-800 shadow-2xs transition-colors"
                  >
                    Sign Up
                  </Link>
                </div>
