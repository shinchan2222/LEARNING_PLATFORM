...
export async function GET() {
  try {
    return NextResponse.json(getDb().prepare('SELECT * FROM team_members ORDER BY id ASC').all());
  } catch (error) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = getDb().prepare(
      'INSERT INTO team_members (name, role, bio, initials, color) VALUES (?, ?, ?, ?, ?)'
    ).run(body.name, body.role, body.bio, body.initials, body.color || '#0B63E5');
